...
GITHUB_NB = "https://colab.research.google.com/github/alexar76/aimarket-school/blob/main/notebooks"
...
# Canonical home of the lessons. The nested mirror (modeldev…/school/) serves
# byte-identical pages, so it must canonicalize here instead of competing with
# the portal in the index.
CANON_BASE = os.environ.get("SCHOOL_CANONICAL_BASE", "https://edu.modelmarket.dev").rstrip("/")
CANON_MOUNT = os.environ.get("SCHOOL_CANONICAL_MOUNT", "").strip().strip("/")

# Hub endpoints used by the browser "Try it" demos. HUB_PROXY is a same-origin
# nginx bridge (deploy/nginx/{edu,modeldev}…conf). The client does NOT fall back
# to a cross-origin public hub call — that is the CORS "Failed to fetch" footgun.
HUB_PUBLIC = os.environ.get("SCHOOL_HUB_URL", "https://modelmarket.dev").rstrip("/")
HUB_PROXY = os.environ.get("SCHOOL_HUB_PROXY", "/hub").rstrip("/")
ORACLE_FAMILY = os.environ.get(
    "SCHOOL_ORACLE_URL", "https://oracles.modelmarket.dev/family"
).rstrip("/")
# Optional local mp4 under school/docs/clips/ is NO LONGER preferred — real
# lesson video is YouTube Shorts/Longs from lessons.yaml (youtube_short / youtube_long).
# Canvas playReel stays as a motion fallback when a lesson has no YouTube id yet.
CLIPS_SRC = SCHOOL / "docs" / "clips"
CLIPS_DIR_NAME = "clips"
# On-ramp spine: discover → pay → warden → academy publish (not catalog order).
SPINE_IDS = (
    "agents-in-5-min",
    "ai-pays-ai",
    "poison-check",
...
# og:image assets copied into the site (per-lesson reel still + portal hero).
OG_DIR_NAME = "og"
OG_PORTAL = "portal.png"

# What the previous build generated — lets the site-root cleanup remove pages of
# renamed/deleted lessons without touching anything it does not own.
MANIFEST_NAME = ".school-build.json"
...
.reel canvas, .reel-video, .reel-yt {
  width: 100%; height: 100%; display: block; object-fit: cover; background: #02050c; border: 0;
}
.reel-yt { position: absolute; inset: 0; }
.reel-links { display: flex; flex-wrap: wrap; gap: 10px; justify-content: center; margin-top: 10px; }
.reel-links a { font-size: 0.85rem; }
.reel .cap {
...
DEMO_JS = r"""
const HUB = __HUB_PUBLIC__;
const HUB_PROXY = __HUB_PROXY__;
const FAMILY = __ORACLE_FAMILY__;
// Same-origin /hub bridge only. Never fetch HUB cross-origin from the browser —
// hub CORS is fail-closed and surfaces as TypeError: Failed to fetch.

async function hubFetch(path, init) {
  if (!HUB_PROXY) {
    throw new Error('Hub bridge not configured (SCHOOL_HUB_PROXY empty)');
  }
  const r = await fetch(HUB_PROXY + path, init);
  if (r.status === 404 || r.status === 405) {
    throw new Error('Hub bridge missing on this host — deploy school-hub-bridge.conf');
  }
  if (r.status >= 500) {
    throw new Error('Hub bridge upstream error HTTP ' + r.status);
  }
  return r;
}

function schoolText(key, fallback) {
  const t = (typeof window !== 'undefined' && window.SCHOOL_I18N) || {};
  return t[key] || fallback;
}

async function schoolDemo(kind, outEl) {
  outEl.textContent = schoolText('running', 'Running…');
  try {
    if (kind === 'hub_search') {
      const r = await hubFetch('/ai-market/v2/search?intent=oracle&limit=5');
      ...
      const r = await hubFetch('/ai-market/v2/invoke', {
        method: 'POST', headers: {'content-type': 'application/json'},
        body: JSON.stringify({
          product_id: 'prod-platon', capability_id: 'platon.random@v1',
          source_hub: FAMILY, input: {}
        })
      });
      const t = await r.text();
      // The hub answers 402 only once the free-trial allowance is spent — say
      // which branch the learner is looking at instead of implying a 402.
      const note = r.status === 402
        ? '402 Payment Required → open a channel and retry'
        : (r.status < 300
            ? 'served without a channel (free-trial allowance) — priced calls settle from a channel'
            : 'hub refused this call — see the body');
      outEl.textContent = `HTTP ${r.status} · ${note}\n${t.slice(0, 500)}`;
    } else if (kind === 'hub_stats' || kind === 'factory_health') {
      const r = await hubFetch('/.well-known/ai-market.json');
      const d = await r.json();
      ...
      const r = await hubFetch(`/ai-market/v2/search?intent=${intent}&limit=6`);
      const d = await r.json();
      outEl.textContent = (d.matches || []).map(m => `${m.capability_id}\n  ${m.description || ''}`).join('\n\n') || tSlice(d);
    } else if (kind === 'basescan_escrow') {
      outEl.textContent = 'Opening Basescan escrow…';
      window.open('https://basescan.org/address/0x12Db8FAC81E5999D2f2087B79e38951571562CF2', '_blank', 'noopener');
      outEl.textContent = 'Escrow 0x12dB8faC…62cF2 — Base mainnet.\nExternal depositor proof: onchain-journal §3j.';
    ...
    } else if (kind === 'gaia_atlas_live') {
      const [gh, inv, ah] = await Promise.all([
        fetch('https://iot.modelmarket.dev/health').then(r => r.json()),
        fetch('https://iot.modelmarket.dev/ai-market/v2/invoke', {
          method: 'POST', headers: {'content-type': 'application/json'},
          body: JSON.stringify({capability_id: 'gaia.weather.read@v1', input: {device_id: 'om-wx-01'}})
        }).then(r => r.json()),
        fetch('https://atlas.modelmarket.dev/health').then(r => r.json())
      ]);
      const reading = (inv.output || {}).reading || {};
      outEl.textContent = [
        `GAIA devices=${gh.devices} sim_control=${gh.sim_control}`,
        `LIVE pin ${reading.device_id} · ${reading.model}`,
        `values ${JSON.stringify(reading.values || {})}`,
        `attestation ${(inv.output || {}).attestation?.algorithm || '—'}`,
        `ATLAS stations=${ah.stations} gaia=${ah.gaia}`
      ].join('\n');
    } else if (kind === 'federation_peers') {
      const [wk, peersBody] = await Promise.all([
        hubFetch('/.well-known/ai-market.json').then(r => r.json()),
        hubFetch('/ai-market/v2/federation/peers').then(r => r.json())
      ]);
      const peers = peersBody.peers || [];
      outEl.textContent = [
        `hub ${wk.name} · federated ${wk.federated_capabilities_count}`,
        `peers ${peers.length}`,
        ...peers.slice(0, 6).map(p => {
          const d = p.declared || {};
          return `  ${p.url}\n    status=${p.status} caps=${p.capabilities_count} trust=${p.trust_score}\n    declared=${d.id || '—'}`;
        })
      ].join('\n');
    } else if (kind === 'hybrid_seal') {
      const wk = await hubFetch('/.well-known/ai-market.json').then(r => r.json());
      const sig = wk.signature || {};
      const hybrid = !!(sig.algorithm && sig.pq_algorithm && sig.value && sig.pq_value);
      outEl.textContent = [
        `classical ${sig.algorithm} pk=${(sig.public_key || '').slice(0, 24)}…`,
        `pq ${sig.pq_algorithm} pq_value=${!!sig.pq_value}`,
        `hybrid_envelope ${hybrid}`,
        hybrid ? 'gate: accept dual orbits' : 'gate: refuse — missing orbit'
      ].join('\n');
    } else {
    ...
    outEl.textContent = [
      schoolText('demo_failed', 'Live hub bridge unavailable.') + ' (' + detail + ')',
      schoolText('demo_failed_hint', 'Open the Colab notebook below — the same calls run there against modelmarket.dev.'),
      'Public hub (for notebooks/CLI): ' + HUB
    ].join('\n\n');
  }
}
...
function playReel(canvas, beats, capEl, barEl, scene) {
  ...
  const scenes = {
    ...
    gaia_atlas_live(u, bi) {
      nebula(u, [[0.45,0.4,0.55,C.cyan,0.16],[0.7,0.65,0.4,C.lime,0.12],[0.25,0.75,0.3,C.hot,0.08]]);
      stars(u, 35);
      const cx = 0.5*W, cy = 0.46*H, R = 0.28*Math.min(W,H);
      // globe
      const g = ctx.createRadialGradient(cx - R*0.3, cy - R*0.3, R*0.1, cx, cy, R);
      g.addColorStop(0, hexA(C.cyan, 0.35)); g.addColorStop(0.7, hexA('#0a2040', 0.9)); g.addColorStop(1, hexA(C.ink, 1));
      ctx.fillStyle = g; ctx.beginPath(); ctx.arc(cx, cy, R, 0, Math.PI*2); ctx.fill();
      ctx.strokeStyle = hexA(C.cyan, 0.45); ctx.lineWidth = 2*dpr;
      ctx.beginPath(); ctx.arc(cx, cy, R, 0, Math.PI*2); ctx.stroke();
      // latitude lines
      for (let i = -2; i <= 2; i++) {
        ctx.strokeStyle = hexA(C.cyan, 0.15); ctx.beginPath();
        ctx.ellipse(cx, cy + i * R * 0.22, R * Math.sqrt(Math.max(0.05, 1 - (i*0.22)**2)), R * 0.18, 0, 0, Math.PI*2); ctx.stroke();
      }
      // LIVE pins
      const pins = [[0.35,0.38],[0.62,0.34],[0.48,0.55],[0.68,0.58],[0.32,0.62]];
      pins.forEach((p, i) => {
        const pulse = 0.5 + 0.5 * Math.sin(u * Math.PI * 4 + i);
        const x = p[0]*W, y = p[1]*H;
        const glow = ctx.createRadialGradient(x, y, 0, x, y, (10 + pulse*8)*dpr);
        glow.addColorStop(0, hexA(i % 2 ? C.lime : C.hot, 0.9));
        glow.addColorStop(1, hexA(C.lime, 0));
        ctx.fillStyle = glow; ctx.beginPath(); ctx.arc(x, y, (10+pulse*8)*dpr, 0, Math.PI*2); ctx.fill();
        ctx.fillStyle = '#fff'; ctx.beginPath(); ctx.arc(x, y, 2.5*dpr, 0, Math.PI*2); ctx.fill();
      });
      if (bi >= 1) {
        ctx.fillStyle = hexA(C.lime, 0.9); ctx.font = `bold ${12*dpr}px ui-monospace,monospace`;
        ctx.fillText('LIVE', cx - 16*dpr, cy + R + 28*dpr);
      }
      if (bi >= 2) {
        ctx.fillStyle = hexA(C.gold, 0.85); ctx.font = `${11*dpr}px ui-monospace,monospace`;
        ctx.fillText('provenance ✓', cx - 42*dpr, 0.12*H);
      }
    },
    federation_peers(u, bi) {
      nebula(u, [[0.5,0.35,0.5,C.violet,0.14],[0.25,0.6,0.35,C.cyan,0.1],[0.75,0.7,0.3,C.gold,0.08]]);
      const hubs = [[0.5,0.28,'HUB'],[0.22,0.55,'A'],[0.78,0.52,'B'],[0.38,0.78,'C'],[0.68,0.80,'D']];
      // edges
      ctx.lineWidth = 1.5*dpr;
      hubs.slice(1).forEach((h, i) => {
        const pending = bi < 2 && i === 2;
        ctx.strokeStyle = hexA(pending ? C.hot : C.cyan, pending ? 0.35 : 0.45);
        ctx.setLineDash(pending ? [5*dpr, 5*dpr] : []);
        ctx.beginPath(); ctx.moveTo(hubs[0][0]*W, hubs[0][1]*H); ctx.lineTo(h[0]*W, h[1]*H); ctx.stroke();
        ctx.setLineDash([]);
      });
      // announce pulse
      const pulseR = ((u * 2) % 1) * 0.35 * Math.min(W,H);
      ctx.strokeStyle = hexA(C.lime, 0.35); ctx.beginPath();
      ctx.arc(hubs[0][0]*W, hubs[0][1]*H, pulseR, 0, Math.PI*2); ctx.stroke();
      hubs.forEach((h, i) => {
        const quarantine = bi >= 1 && i === 3;
        const x = h[0]*W, y = h[1]*H;
        if (quarantine) {
          ctx.strokeStyle = hexA(C.hot, 0.7 + 0.2*Math.sin(u*12)); ctx.lineWidth = 2*dpr;
          ctx.strokeRect(x - 22*dpr, y - 22*dpr, 44*dpr, 44*dpr);
        }
        const col = i === 0 ? C.gold : (quarantine ? C.hot : C.cyan);
        const g = ctx.createRadialGradient(x,y,0,x,y,18*dpr);
        g.addColorStop(0, hexA(col, 0.95)); g.addColorStop(1, hexA(col, 0));
        ctx.fillStyle = g; ctx.beginPath(); ctx.arc(x,y,16*dpr,0,Math.PI*2); ctx.fill();
        ctx.fillStyle = '#fff'; ctx.font = `bold ${10*dpr}px ui-monospace,monospace`;
        ctx.fillText(h[2], x - 6*dpr, y + 4*dpr);
      });
      if (bi >= 2) {
        ctx.fillStyle = hexA(C.lime, 0.9); ctx.font = `${11*dpr}px ui-monospace,monospace`;
        ctx.fillText('approved · crawl', 0.32*W, 0.12*H);
      }
    },
    hybrid_seal(u, bi) {
      nebula(u, [[0.5,0.45,0.55,C.violet,0.16],[0.3,0.3,0.35,C.cyan,0.12],[0.7,0.7,0.3,C.gold,0.1]]);
      const cx = 0.5*W, cy = 0.48*H;
      const R1 = 52*dpr, R2 = 78*dpr;
      const a1 = u * Math.PI * 4, a2 = -u * Math.PI * 3.2;
      // dual orbits
      [[R1, C.cyan, a1, 'Ed25519'], [R2, C.violet, a2, 'ML-DSA']].forEach(([R, col, a, label], i) => {
        ctx.strokeStyle = hexA(col, 0.55); ctx.lineWidth = 2.5*dpr;
        ctx.beginPath(); ctx.arc(cx, cy, R, 0, Math.PI*2); ctx.stroke();
        const x = cx + Math.cos(a)*R, y = cy + Math.sin(a)*R;
        const g = ctx.createRadialGradient(x,y,0,x,y,14*dpr);
        g.addColorStop(0, hexA(col, 1)); g.addColorStop(1, hexA(col, 0));
        ctx.fillStyle = g; ctx.beginPath(); ctx.arc(x,y,12*dpr,0,Math.PI*2); ctx.fill();
        // trail
        for (let t = 1; t <= 8; t++) {
          const aa = a - t * 0.12 * (i ? -1 : 1);
          ctx.fillStyle = hexA(col, 0.08 * (9-t));
          ctx.beginPath(); ctx.arc(cx + Math.cos(aa)*R, cy + Math.sin(aa)*R, 3*dpr, 0, Math.PI*2); ctx.fill();
        }
        if (bi >= 1) {
          ctx.fillStyle = hexA(col, 0.85); ctx.font = `${10*dpr}px ui-monospace,monospace`;
          ctx.fillText(label, cx - 24*dpr, cy + (i ? R2 + 22*dpr : -R1 - 14*dpr));
        }
      });
      // receipt crystal
      ctx.fillStyle = hexA(C.gold, 0.9);
      ctx.beginPath();
      ctx.moveTo(cx, cy - 16*dpr); ctx.lineTo(cx + 12*dpr, cy); ctx.lineTo(cx, cy + 16*dpr); ctx.lineTo(cx - 12*dpr, cy);
      ctx.closePath(); ctx.fill();
      if (bi >= 2) {
        ctx.strokeStyle = hexA(C.lime, 0.95); ctx.lineWidth = 3*dpr;
        ctx.beginPath(); ctx.moveTo(cx - 18*dpr, cy + 40*dpr); ctx.lineTo(cx - 4*dpr, cy + 52*dpr); ctx.lineTo(cx + 22*dpr, cy + 32*dpr); ctx.stroke();
        ctx.fillStyle = hexA(C.lime, 0.9); ctx.font = `${11*dpr}px ui-monospace,monospace`;
        ctx.fillText('dual orbit · sealed', cx - 48*dpr, 0.12*H);
      }
    }
  };
...
def _script_json(value: Any) -> str:
    """JSON for an inline <script>: `</` can never terminate the element early."""
    return json.dumps(value, ensure_ascii=False).replace("</", "<\\/")


def demo_js() -> str:
    """DEMO_JS with the hub endpoints of this build injected."""
    return (
        DEMO_JS.replace("__HUB_PUBLIC__", json.dumps(HUB_PUBLIC))
        .replace("__HUB_PROXY__", json.dumps(HUB_PROXY))
        .replace("__ORACLE_FAMILY__", json.dumps(ORACLE_FAMILY))
    )
...
def _mounted(mount: str, sub: str) -> str:
    """Join a mount segment and a lesson subpath: ('school', '/x/') → '/school/x/'."""
    if not sub.startswith("/"):
        sub = "/" + sub
    if not sub.endswith("/"):
        sub += "/"
    m = f"/{mount}" if mount else ""
    if sub == "/":
...
def canon_path(sub: str) -> str:
    """Path of a page on THIS build's site: '/' or '/school/agents-in-5-min/'."""
    return _mounted(MOUNT, sub)


def canonical_url(lang: str, sub: str) -> str:
    """SEO canonical for a page — always the portal, never the nested mirror."""
    return f"{CANON_BASE}{lang_prefix(lang)}{_mounted(CANON_MOUNT, sub)}"


def is_canonical_site() -> bool:
    """True when this build IS the canonical site (owns robots.txt / sitemap)."""
    return BASE == CANON_BASE and MOUNT == CANON_MOUNT


def og_image_url(name: str) -> str:
    """Absolute og:image on the page's own origin (assets shipped per site)."""
    return f"{BASE}{mount_prefix()}/{OG_DIR_NAME}/{name}"
...
def fonts_href(lang: str, *, depth: int = 0) -> str:
    """Self-hosted faces at the school tree root (ecosystem-landing/fonts/)."""
    if lang == "en":
        return "../" * (1 + depth) + "fonts/fonts.css"
    return "../" * (2 + depth) + "fonts/fonts.css"
...
def workshop_url(lang: str) -> str:
    name = {
        ...
        "fr": "workshop.fr.md",
        "zh": "workshop.zh.md",
    }.get(lang, "workshop.md")
    return f"https://github.com/alexar76/hestia/blob/main/docs/{name}"
...
def write_notebook(lesson: dict[str, Any]) -> None:
    """Write a Colab notebook: markdown intro + one or more code cells.

    Split ``colab_cells`` on a lone ``# ---`` line into separate code cells so
    lessons can ship a short lab sequence instead of a single 10-line dump.
    """
    NB_OUT.mkdir(parents=True, exist_ok=True)
    academy = academy_url(lesson["academy"])
    beats = lesson.get("short_beats") or []
    beats_md = "\n".join(f"- {b}" for b in beats) if beats else "- Run the cells top to bottom"
    md = (
        f"# {lesson['title']}\n\n"
        f"{lesson['punch']}\n\n"
        f"**~{lesson.get('minutes', 5)} min · free · live network**\n\n"
        f"{beats_md}\n\n"
        f"**Academy next:** [{lesson['academy_label']}]({academy})\n\n"
        f"_Tip: Runtime → Run all. Hub calls use the public AIMarket surface — no API key._\n"
    )
    raw_lines = list(lesson["colab_cells"])
    ...
    code_chunks = ["\n".join(c).strip() + "\n" for c in chunks if any(x.strip() for x in c)]
    if not code_chunks:
        code_chunks = ["print('empty notebook')\n"]
    ...
    cells.append(
        {
            ...
            "source": [
                f"## Next\n\n"
                f"1. Open the Academy: [{lesson['academy_label']}]({academy})\n"
                f"2. Fill `# YOUR CODE HERE` stubs in `courselib/exercises.py`\n"
                f"3. `python labs/run_exercises.py --certificate \"Your Name\"`\n"
            ],
        }
    )
    nb = {
...
def hreflang_tags(sub: str) -> str:
    """Alternates point at the canonical portal, not at the mirror copy."""
    tags = []
    for L in LANGS:
        tags.append(
            f'<link rel="alternate" hreflang="{L}" href="{html.escape(canonical_url(L, sub))}" />'
        )
    tags.append(
        f'<link rel="alternate" hreflang="x-default" href="{html.escape(canonical_url("en", sub))}" />'
    )
    return "\n".join(tags)


def nav(lang: str, U: dict[str, str], path: str, *, on_portal: bool = False) -> str:
    lessons_current = ' aria-current="page"' if on_portal else ""
  ...
  <div class="nav-links">
    <a href="{html.escape(school_href(lang, '/'))}"{lessons_current}>{html.escape(U['nav_lessons'])}</a>
    ...
    <a href="{html.escape(workshop_url(lang))}" target="_blank" rel="noopener">{html.escape(U['nav_workshop'])}</a>
...
def head(
    ...
    *,
    depth: int = 0,
    og_image: str = OG_PORTAL,
) -> str:
    url = canonical_url(lang, sub)
    image = og_image_url(og_image)
    # summary_large_image expects a wide (~1.91:1) image. The portal hero is
    # 1920×995; the per-lesson reel stills are 9:16 portrait, which that card
    # crops to a letterboxed sliver — those pages use the square `summary` card.
    card = "summary_large_image" if og_image == OG_PORTAL else "summary"
...
<meta property="og:site_name" content="AIMarket School" />
<meta property="og:locale" content="{html.escape(lang)}" />
...
<link href="{html.escape(fonts_href(lang, depth=depth))}" rel="stylesheet" />
...
def lesson_page(
    ...
    beats_json = _script_json(beats)
    by_id = {L["id"]: L for L in lessons}
    nav_lessons = ""
    if lesson["id"] in SPINE_IDS:
        prev_l = _spine_prev(lesson["id"], by_id)
        next_l = _spine_next(lesson["id"], by_id)
        if prev_l:
            nav_lessons += (
        ...
        elif lesson["id"] == SPINE_IDS[-1]:
            cta = U.get("spine_next_publish") or "Academy: publish a capability ↗"
            nav_lessons += (
                f'<a class="btn btn-primary" href="{html.escape(academy_url("ai-factory-course"))}" '
                f'target="_blank" rel="noopener">{html.escape(cta)}</a>'
            )
    else:
    ...
    workshop_btn = ""
    if lesson.get("workshop"):
        workshop_btn = (
            f'<a class="btn btn-ghost" href="{html.escape(workshop_url(lang))}" '
            f'target="_blank" rel="noopener">{html.escape(U["cta_workshop"])}</a>'
        )
    demo_i18n = _script_json(
        {
            "running": U.get("running") or "Running…",
            "demo_failed": U.get("demo_failed") or "",
            "demo_failed_hint": U.get("demo_failed_hint") or "",
        }
    )
    yt_short = _youtube_id(lesson.get("youtube_short") or "")
    yt_long = _youtube_id(lesson.get("youtube_long") or "")
    # Reel is always the programmatic canvas motion — YouTube is links underneath.
    reel_inner = """<div class="progress"><i id="reel-bar"></i></div>
        <canvas id="reel-c"></canvas>
        <div class="cap" id="reel-cap"></div>"""
    reel_note = U["short_note"]
    yt_links = []
    if yt_short:
        yt_links.append(
            f'<a class="btn btn-ghost" href="https://youtu.be/{html.escape(yt_short)}" '
            f'target="_blank" rel="noopener">{html.escape(U.get("youtube_short") or "Short ↗")}</a>'
        )
    if yt_long:
        yt_links.append(
            f'<a class="btn btn-ghost" href="https://youtu.be/{html.escape(yt_long)}" '
            f'target="_blank" rel="noopener">{html.escape(U.get("youtube_long") or "Full video ↗")}</a>'
        )
    reel_links = f'<div class="reel-links">{"".join(yt_links)}</div>' if yt_links else ""
    reel_boot = f"""(() => {{
  const beats = {beats_json};
  const scene = {_script_json(lesson.get("demo") or lesson["id"])};
  ...
  document.getElementById('run-demo').addEventListener('click', () => {{
    schoolDemo({_script_json(lesson.get('demo'))}, document.getElementById('demo-out'));
  }});
}})();"""
    return (
        head(
            f"{copy['title']} — AIMarket School",
            copy["punch"],
            sub,
            lang,
            depth=1,
            og_image=f"{lesson['id']}.png",
        )
        + nav(lang, U, path)
        + f"""
<div class="wrap section lesson-hero">
  ...
  <div class="layout">
    <div>
      ...
      <p class="note" style="text-align:center">{html.escape(reel_note)}</p>
      {reel_links}
    </div>
    <div>
  ...
  <div class="ctas" style="margin-top:28px">{nav_lessons}{workshop_btn}<a class="btn btn-ghost" href="../">{html.escape(U['all_lessons'])}</a></div>
</div>
{foot(lang, U)}
<script>
window.SCHOOL_I18N = {demo_i18n};
{demo_js()}
{reel_boot}
</script>
</body></html>
...
def _youtube_id(url_or_id: str) -> str:
    """Accept youtu.be/ID, youtube.com/watch?v=, youtube.com/shorts/ID, or bare id."""
    s = (url_or_id or "").strip()
    if not s:
        return ""
    if "youtu.be/" in s:
        return s.rsplit("youtu.be/", 1)[-1].split("?", 1)[0].split("&", 1)[0]
    if "youtube.com/shorts/" in s:
        return s.rsplit("/shorts/", 1)[-1].split("?", 1)[0].split("&", 1)[0]
    if "v=" in s:
        return s.split("v=", 1)[-1].split("&", 1)[0]
    # bare id
    if all(c.isalnum() or c in "-_" for c in s) and 6 <= len(s) <= 20:
        return s
    return ""


def _spine_next(lesson_id: str, by_id: dict[str, dict[str, Any]]) -> dict[str, Any] | None:
    if lesson_id not in SPINE_IDS:
        return None
    i = SPINE_IDS.index(lesson_id)
    if i + 1 >= len(SPINE_IDS):
        return None
    return by_id.get(SPINE_IDS[i + 1])


def _spine_prev(lesson_id: str, by_id: dict[str, dict[str, Any]]) -> dict[str, Any] | None:
    if lesson_id not in SPINE_IDS:
        return None
    i = SPINE_IDS.index(lesson_id)
    if i <= 0:
...
def _clip_href(lang: str, lesson_id: str) -> str:
    """Relative href from a lesson page to /clips/{id}.mp4."""
    name = f"{CLIPS_DIR_NAME}/{lesson_id}.mp4"
    if MOUNT:
        # /school/{id}/ or /{lang}/school/{id}/ → ../clips/
        return f"../{name}"
    if lang == "en":
        return f"/{name}"
    return f"../{name}"
...
def _copy_clips(out: Path) -> None:
    """No-op: lesson video is YouTube (lessons.yaml). Local mp4 placeholders are not shipped."""
    del out  # kept so call sites stay stable; assets live on YouTube.
...
def portal_page(
  ...
  <div class="meta">→ {html.escape(c['academy_label'])}</div>
    ...
    path = canon_path("/")
    return (
        head(U["portal_title"], portal["lede"], "/", lang, og_image=OG_PORTAL)
        + nav(lang, U, path, on_portal=True)
        + f"""
<header class="hero-cosmic">
  ...
  <div class="wrap">
    ...
    <div class="ctas">
      <a class="btn btn-primary" href="{html.escape(SPINE_IDS[0])}/">{html.escape(U.get('cta_spine') or U['cta_start'])}</a>
      <a class="btn btn-ghost" href="{html.escape(learn_href(lang))}">{html.escape(U['cta_academy'])}</a>
      <a class="btn btn-ghost" href="{html.escape(workshop_url(lang))}" target="_blank" rel="noopener">{html.escape(U['cta_workshop'])}</a>
    </div>
    <p class="note" style="margin-top:14px;max-width:42ch">{html.escape(U.get('spine_blurb') or '')}</p>
  </div>
</header>
<main class="wrap">
...
def build_lang(
    ...
    if not MOUNT and lang == "en":
        ...
        # Targeted cleanup: only what this builder owns — a blanket wipe of every
        # non-language entry would silently eat hand-added site files (CNAME,
        # extra assets, ACME webroot…) on the dedicated portal. The manifest adds
        # entries from PREVIOUS builds, so a renamed or deleted lesson does not
        # keep serving a stale page at its old canonical URL.
        owned = {
            "index.html",
            "school.css",
            "notebooks",
            ...
            "sitemap.xml",
            MANIFEST_NAME,
        } | lesson_ids | read_manifest(out)
        for p in list(out.iterdir()):
            if p.name in LANGS or p.name not in owned:
                continue
            if p.is_dir():
                shutil.rmtree(p)
            else:
...
def read_manifest(root: Path) -> set[str]:
    """Entries the previous build created in this directory (may be empty)."""
...
def write_manifest(root: Path, lessons: list[dict[str, Any]]) -> None:
    entries = sorted(
        {"index.html", "school.css", "notebooks", OG_DIR_NAME, "robots.txt", "sitemap.xml"}
        | {L["id"] for L in lessons}
    )
    (root / MANIFEST_NAME).write_text(
        json.dumps({"generator": "school/build.py", "entries": entries}, indent=1) + "\n",
        encoding="utf-8",
    )


def copy_og_assets(root: Path, lessons: list[dict[str, Any]]) -> int:
    """Ship the reel stills that og:image / twitter:image point at."""
    og = root / OG_DIR_NAME
    if og.exists():
        shutil.rmtree(og)
    og.mkdir(parents=True, exist_ok=True)
    copied = 0
    hero = SCHOOL / "docs" / "recordings" / "school-portal-hero.png"
    if hero.is_file():
        shutil.copy2(hero, og / OG_PORTAL)
        copied += 1
    for L in lessons:
        src = SCHOOL / "docs" / "screenshots" / f"{L['id']}.png"
        if src.is_file():
            shutil.copy2(src, og / f"{L['id']}.png")
            copied += 1
    return copied


def sitemap_url() -> str:
    m = f"/{CANON_MOUNT}" if CANON_MOUNT else ""
    return f"{CANON_BASE}{m}/sitemap.xml"


def write_robots(root: Path) -> None:
    (root / "robots.txt").write_text(
        "# AIMarket School — clip lessons portal\n"
        "User-agent: *\n"
        "Allow: /\n"
        "\n"
...
def write_sitemap(root: Path, lessons: list[dict[str, Any]]) -> None:
    """Canonical URLs only (no lastmod → the build stays reproducible)."""
    subs = ["/"] + [f"/{L['id']}/" for L in lessons]
    out = [
    ...
    for sub in subs:
        for lang in LANGS:
            out.append("  <url>")
            out.append(f"    <loc>{canonical_url(lang, sub)}</loc>")
            for alt in LANGS:
                out.append(
                    f'    <xhtml:link rel="alternate" hreflang="{alt}"'
                    f' href="{canonical_url(alt, sub)}" />'
                )
            out.append(
                '    <xhtml:link rel="alternate" hreflang="x-default"'
                f' href="{canonical_url("en", sub)}" />'
            )
            out.append("    <changefreq>monthly</changefreq>")
            out.append(f"    <priority>{'1.0' if sub == '/' else '0.8'}</priority>")
            out.append("  </url>")
    out.append("</urlset>")
...
def main() -> int:
    ...
    en_root = school_root("en")
    og_count = copy_og_assets(en_root, lessons)
    seo = "canonical"
    if is_canonical_site():
        write_robots(en_root)
        write_sitemap(en_root, lessons)
    if not MOUNT:
        # Site-root builds share the directory with hand-added files, so record
        # what we own for the next run's cleanup.
        write_manifest(en_root, lessons)
    else:
        # Mirror build: pages canonicalize to CANON_BASE, so it must not ship a
        # competing sitemap/robots for the same content.
        seo = f"mirror → canonical {CANON_BASE}"

    where = f"{OUT_ROOT}" + (f" (mount {mount_prefix() or '/'})" )
    print(
        f"OK school → {where} langs={LANGS} lessons={len(lessons)} base={BASE} "
        f"og={og_count} seo={seo} hub={HUB_PROXY or '-'}→{HUB_PUBLIC}"
    )
    return 0
