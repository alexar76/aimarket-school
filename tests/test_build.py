...
def test_thirteen_lessons_with_unique_ids_and_orders(lessons):
    assert len(lessons) == 13
    assert len({L["id"] for L in lessons}) == 13
    assert [L["order"] for L in lessons] == list(range(1, 14))
...
def test_operator_workshop_is_linked_from_the_street(builds, lessons):
    """Not a 14th lesson: School portal + pay/knock pages point at the HESTIA runbook."""
    flagged = {L["id"] for L in lessons if L.get("workshop")}
    assert flagged == {"ai-pays-ai", "hubs-that-knock"}
    portal = (builds["portal"] / "index.html").read_text(encoding="utf-8")
    assert "https://github.com/alexar76/hestia/blob/main/docs/workshop.md" in portal
    assert "HESTIA workshop" in portal
    ru = (builds["portal"] / "ru" / "index.html").read_text(encoding="utf-8")
    assert "workshop.ru.md" in ru
    pay = (builds["portal"] / "ai-pays-ai" / "index.html").read_text(encoding="utf-8")
    knock = (builds["portal"] / "hubs-that-knock" / "index.html").read_text(encoding="utf-8")
    physics = (builds["portal"] / "physics-beats-blackbox" / "index.html").read_text(
        encoding="utf-8"
    )
    assert "90 min on a HESTIA host" in pay
    assert "90 min on a HESTIA host" in knock
    assert "90 min on a HESTIA host" not in physics
